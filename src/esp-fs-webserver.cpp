...
#if ESP_FS_WS_USE_SSDP
    this->on("/description.xml", HTTP_GET, [this]()  {
...
void FSWebServer::getStatus() {
    ...
    doc["liburl"] = LIB_URL;
#if ESP_FS_WS_USE_SD
    doc["sdcard_mounted"] = sdcard_mounted;
    doc["sdcard_inserted"] = sdcard_inserted;
#endif
    String reply;
...
void FSWebServer::handleSetup() {
    ...
    this->send_P(200, "text/html", (const char*)SETUP_HTML, sizeof(SETUP_HTML));
#else
    replyToCLient(NOT_FOUND, PSTR("FILE_NOT_FOUND"));
#endif
}
...
bool FSWebServer::handleFileRead(const char* uri) {
    ...
    if (path.startsWith("/sd")) {
        ...
        if (SD.exists(sdPath.c_str())) {
            file = SD.open(sdPath.c_str(), FILE_READ);
            path  = sdPath;
        } else {
            path = sdPath + ".gz";
