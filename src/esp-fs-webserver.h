...
#ifndef ESP_FS_WS_USE_SD
    #define ESP_FS_WS_USE_SD             1   // try to mount and serve the SD card
#endif

#ifndef ESP_FS_WS_USE_SSDP
    #define ESP_FS_WS_USE_SSDP            1   // try to mount and serve the SD card
    #include <ESP32SSDP.h>
#endif
  ...
  public:
    ...
    */
    ...
    inline bool broadcastWebSocket(const String& payload) {
        return broadcastWebSocket(payload.c_str());
    }
    inline bool sendWebSocket(uint8_t num, const String& payload) {
        return sendWebSocket(num, payload.c_str());
    }
    ...
    */
    ...
    bool closeConfiguration( bool write = true) {
        return setup->closeConfiguration(write);
    }
    bool optionToFile(const char* f, const char* id, bool ow) {
        return setup->optionToFile(f, id, ow);
    }
    void addHTML(const char* h, const char* id, bool ow = false) {
        setup->addHTML(h, id, ow);
    }
    void addCSS(const char* c, const char* id, bool ow = false) {
        setup->addCSS(c, id, ow);
    }
    void addJavascript(const char* s, const char* id, bool ow = false) {
        setup->addJavascript(s, id, ow);
    }
    void addDropdownList(const char *l, const char** a, size_t size) {
        setup->addDropdownList(l, a, size);
    }
    void addOptionBox(const char* title) {
        setup->addOption("param-box", title);
    }
    ...
    bool getOptionValue(const char *lbl, T &var) {
        return setup->getOptionValue(lbl, var);
    }
    template <typename T>
    bool saveOptionValue(const char *lbl, T &var) {
        return setup->saveOptionValue(lbl, var);
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////
#endif
